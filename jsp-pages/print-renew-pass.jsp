...
<%
    ...
    try{
        ...
        boolean isRenewPaymentDone = checkRenewPassPaymentStatus(applicationId, out);
        if(flag && isRenewPaymentDone){
...
<%!
    public static boolean checkRenewPassPaymentStatus(String applicationId, JspWriter out) throws Exception{
        boolean isPaymentDone = false;
        try{
            con = fetchConnection();
            stmt = con.createStatement();
            String selectQuery = "select payment_status from renew_pass where application_form_id = '"+applicationId+"'";
            rs = stmt.executeQuery(selectQuery);
            if(!rs.next()){
                isPaymentDone = false;
            } else {
                int paymentStatus = rs.getInt("payment_status");
                isPaymentDone = (paymentStatus == 1) ? true : false;
            }
        } catch(Exception e){
            out.println(" Check approve & submit status exception: " +e);
        } finally {
...
<%!
    public static void printPass(java.util.List<String> applicationDetails, JspWriter out) throws Exception{
        try{
            ...
            String logoPath = userDirectory + File.separator +"tomcat"+File.separator+ "webapps" + File.separator + "pmt-pass" + File.separator + "images" + File.separator + "logo.gif";
							...
							+ "<center>"
								...
								+ "<table border=1>"
									+ "<thead>"
										...
										+ "</tr>"
										+ "<tr align=center>"
											+ "<td valign=\"top\">Bus Route From:</td>"
											+ "<td valign=\"top\">"+applicationDetails.get(3)+"</td>"
											+ "<td valign=\"top\">Bus Route To:</td>"
										+ "<td valign=\"top\">"+applicationDetails.get(4)+"</td>"
										+ "</tr>"
										
										+ "<tr align=center>"
											+ "<td valign=\"top\">RENEW PASS DATE:</td>"
											+ "<td valign=\"top\">"+passengerDOB+"</td>"
										+ "</tr>"
